...
class ThemeDetector {
	 ...
	 * @param {string} [config.cookieName='theme_preference'] - Name of the cookie for theme preference
	 * @param {number} [config.cookieExpiresMinutes=365*24*60] - Cookie expiration in minutes
	 */
	constructor(cookieManager, config = {}) {
		...
		this.#cookieName = config.cookieName ?? 'theme_preference';
	...
	static checkDarkTheme(cookieManager, config = {}) {
		...
		const cookieName = config.cookieName ?? 'theme_preference';
	...
	isDarkThemeEnabled() {
		...
		if (!this.#isFromAdminPanel && !this.#isDarkThemeEnabled) return false;
		...
		// Fall back to system preference
		return (this.#isSystemThemeEnabled && this.#mediaQuery.matches);
	}
		...
		if (themeSwitcherEl) {
			...
			if (buttonEl && menuItemsEls.length > 0) {
				...
				menuItemsEls.forEach(item => {
					...
					if (itemTheme === 'system' && itemTheme !== buttonTheme) {
						// Update button data and HTML label
						buttonEl.setAttribute('data-theme', 'system');
