	...
	public function apiJsScriptTag(?string $formId = '', array $configuration = []): string
	{
		...
		if ($this->version == 'v3') {
			...
			if ($jsCustomValidation) {
				$validateFunction = $jsCustomValidation ? "{$jsCustomValidation}({$fieldId}, token);" : '';
			} else {
				
				$validationRoute = config('recaptcha.validation_route', 'recaptcha/validate');
				$tokenParameterName = config('recaptcha.token_parameter_name', 'token');
				...
				$validateFunction = "
                fetch('/{$validationRoute}?{$tokenParameterName}=' + token, {
                    headers: {
