...
@php
	...
	if (!empty($togglePassword) && in_array($togglePassword, ['link', 'icon'])) {
		if ($togglePassword == 'link') {
			...
			$labelRightContent .= '<a href="" class="toggle-password-link" ' . $tooltip . ' data-toggle-text="true" data-ignore-guard="true">';
			$labelRightContent .= trans('auth.show');
			$labelRightContent .= '</a>';
		} else {
			$inputGroupClass = 'toggle-password-wrapper';
			$suffix = '<a href="" class="toggle-password-link" data-ignore-guard="true"><i class="fa-regular fa-eye-slash"></i></a>';
		}
	}
