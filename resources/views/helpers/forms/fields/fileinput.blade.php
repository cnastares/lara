...
@php
	...
	$fnFilePreview = function($idx, $file) {
		global $diskName, $defaultKey, $defaultFileUrl, $deleteUrlPattern;
		...
		<div class="{{ $colField }}">
			...
			@if (!$allowsMultiple && $downloadable)
				...
				@if (!empty($fileToDownloadPath) && $disk->exists($fileToDownloadPath))
					<div>
						<a class="btn btn-default" href="{{ $fileToDownloadUrl }}" target="_blank">
