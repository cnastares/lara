...
@php
	...
	// file-preview-frame
	$defaultPreviewFrameWidth = 150;
	$defaultPreviewFrameHeight = 160;
...
@pushonce("{$viewName}_assets_styles")
	<style>
		...
		.file-drop-zone {
			margin: 2px;
			min-height: {{ ($previewFrameHeight * 100)/160 }}px;
		}
		...
		onDocumentReady((event) => {
			...
			avatarFieldEl.on('filebatchpreupload', (event, data) => {
				...
				if (uploadSuccessEl.length) {
					uploadSuccessEl.html('<ul></ul>').hide();
				}
			});
			...
			avatarFieldEl.on('filebatchuploadsuccess', (event, data) => {
				...
				$.each(data.files, (key, file) => {
					if (typeof file !== 'undefined') {
						...
						let fileMessage = `<li>${successfulMessage}</li>`;
			...
			avatarFieldEl.on('filedeleted', (event, key, jqXHR, data) => {
				...
				const uploadSuccessEl = $(elSuccessContainer);
				if (uploadSuccessEl.length) {
					uploadSuccessEl.html('<ul><li></li></ul>').hide();
					uploadSuccessEl.find('ul li').append(message);
					uploadSuccessEl.fadeIn('slow');
				}
			});
		});
