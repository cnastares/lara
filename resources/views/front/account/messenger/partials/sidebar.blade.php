...
<div class="col-md-3 col-lg-2">
	<ul class="nav nav-pills inbox-nav">
		<li class="nav-item{{ (!request()->has('filter') || request()->query('filter')=='') ? ' active' : '' }}">
			<a class="nav-link" href="{{ url(urlGen()->getAccountBasePath() . '/messages') }}">
				{{ t('inbox') }}
				@php
		...
		<li class="nav-item{{ (request()->query('filter')=='unread') ? ' active' : '' }}">
			<a class="nav-link" href="{{ url(urlGen()->getAccountBasePath() . '/messages?filter=unread') }}">
				{{ t('unread') }}
			</a>
		</li>
		<li class="nav-item{{ (request()->query('filter')=='started') ? ' active' : '' }}">
			<a class="nav-link" href="{{ url(urlGen()->getAccountBasePath() . '/messages?filter=started') }}">
				{{ t('started') }}
			</a>
		</li>
		<li class="nav-item{{ (request()->query('filter')=='important') ? ' active' : '' }}">
			<a class="nav-link" href="{{ url(urlGen()->getAccountBasePath() . '/messages?filter=important') }}">
				{{ t('important') }}
			</a>
		</li>
	</ul>
