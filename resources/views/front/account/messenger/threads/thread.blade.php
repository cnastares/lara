...
<div class="list-group-item{{ data_get($thread, 'p_is_unread') ? '' : ' seen' }}">
	...
	<a href="{{ url(urlGen()->getAccountBasePath() . '/messages/' . data_get($thread, 'id')) }}" class="list-box-user">
		<img src="{{ url(data_get($thread, 'p_creator.photo_url', '')) }}" alt="{{ data_get($thread, 'p_creator.name') }}">
	</a>
	<a href="{{ url(urlGen()->getAccountBasePath() . '/messages/' . data_get($thread, 'id')) }}" class="list-box-content">
	...
	<div class="list-box-action">
		@if (data_get($thread, 'p_is_important'))
			<a href="{{ url(urlGen()->getAccountBasePath() . '/messages/' . data_get($thread, 'id') . '/actions?type=markAsNotImportant') }}"
		...
		@else
			<a href="{{ url(urlGen()->getAccountBasePath() . '/messages/' . data_get($thread, 'id') . '/actions?type=markAsImportant') }}"
			   data-bs-toggle="tooltip"
			   data-bs-placement="top"
		...
		@endif
		<a href="{{ url(urlGen()->getAccountBasePath() . '/messages/' . data_get($thread, 'id') . '/delete') }}"
		   data-bs-toggle="tooltip"
		   data-bs-placement="top"
		   title="{{ t('Delete') }}"
		>
		...
		@if (data_get($thread, 'p_is_unread'))
			<a href="{{ url(urlGen()->getAccountBasePath() . '/messages/' . data_get($thread, 'id') . '/actions?type=markAsRead') }}"
			   class="markAsRead"
			   data-bs-toggle="tooltip"
			   data-bs-placement="top"
		...
		@else
			<a href="{{ url(urlGen()->getAccountBasePath() . '/messages/' . data_get($thread, 'id') . '/actions?type=markAsUnread') }}"
			   class="markAsRead"
			   data-bs-toggle="tooltip"
			   data-bs-placement="top"
